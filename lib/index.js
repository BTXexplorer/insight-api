...
/**
 * A service for btxcore to enable HTTP routes to query information about the blockchain.
 *
...
var InsightAPI = function(options) {
  ...
  this.coinTicker = options.coinTicker || 'https://api.coinmarketcap.com/v1/ticker/bitcore/?convert=USD';
...
InsightAPI.dependencies = ['bitcored', 'web'];
...
InsightAPI.prototype.start = function(callback) {
  ...
  this.node.services.bitcored.on('tx', this.transactionEventHandler.bind(this));
  this.node.services.bitcored.on('block', this.blockEventHandler.bind(this));
