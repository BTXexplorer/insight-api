...
UtilsController.prototype.estimateSmartFee = function(req, res) {
  ...
  async.map(nbBlocks, function(n, next) {
    var num = parseInt(n);
    // Insight and bitcore JSON-RPC return bitcore for this value (instead of satoshis).
    self.node.services.bitcored.estimateSmartFee(num, conservative, function(err, fee) {
      if (err) {
        return next(err);
      }
      next(null, [num, fee]);
